...
def lbx_on_select_com_port(self,Event=None):
    ...
    if connected==0:
        try:
            ...
            connected=1
            #lbx_com_port_list.config(state='disabled')
...
for i in range(0,num_of_strings):   
    ...
    ent_from_freq[i].insert(0,sheet.Cells(i+1,1).value)    
    ent_to_freq[i].insert(0,sheet.Cells(i+1,2).value)
    ent_corr_cap[i].insert(0,sheet.Cells(i+1,3).value)
    lbl_from_freq.append(Label(frm[i],text="Frequency from"))
...
#messagebox.showinfo("debug", num_of_strings)
